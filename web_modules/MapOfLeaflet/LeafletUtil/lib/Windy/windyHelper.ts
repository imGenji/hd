...
export const invert = (x: number, y: number, windy: Windy) => {
    // @ts-ignore
 ...
 */
export const isValue = function(x: any) {
 ...
 */
export const floorMod = function(a: number, n: number) {
 ...
 */
export const distort = function(projection: object, λ: number, φ: number, x: number, y: number, scale: number, wind: [number, number, number], windy: Windy) {
 ...
 */
const clamp = function(x: number, range: [number, number]) {
    return Math.max(range[0], Math.min(x, range[1]));
};


const mercY = function( lat : number) {
    return Math.log( Math.tan( lat / 2 + Math.PI / 4 ) );
};

const rad2deg = function( ang: number ){
    return ang / (Math.PI/180.0);
};

const distortion = function(projection: object, λ: number, φ: number, x: number, y: number, windy: Windy) {
    const τ = 2 * Math.PI;
...
const project = function(lat: number, lon: number, windy: Windy) { // both in radians, use deg2rad if neccessary
