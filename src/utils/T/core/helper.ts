 ...
 * @param values 排除的值
 */
export const difference = <T>(data: T[], values: T[]): T[] => data.filter(item => !values.includes(item));
 ...
 * @param defaultCom
 */
export const showComponent = (isShow: boolean, com: React.ReactNode, defaultCom?: React.ReactNode) => {
    if(isShow){
        return com;
    }else {
        if(typeof defaultCom !== "undefined") return defaultCom
        return null
    }
}
